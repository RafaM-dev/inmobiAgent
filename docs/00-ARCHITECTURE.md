...
| **Back-office** | **Cliente HTTP, bloques de mensaje, máquina de sesión, flujo SSE y pantalla de acceso** | **Vitest + jsdom + Testing Library (D72)** |
...
| D72 | **El panel se prueba por lo que se rompe callado, no por su aspecto.** Nada de comprobar clases CSS ni capturas de pantalla: eso falla en cada retoque de diseño y no falla nunca cuando importa. Se prueban cinco cosas, y cada una porque su fallo es invisible hasta que un asesor toma una decisión equivocada: la respuesta de la API validada contra el contrato compartido —con el error saliendo ahí, con ruta y campo, en vez de tres componentes más abajo—; que los precios se pinten **exactamente** como los mandó el backend, sin reformatear, porque el asesor tiene que ver lo mismo que vio el cliente; que la sesión no se quede a medias tras un acceso fallido ni colgada en `loading` si la API está caída; que un evento SSE ilegible no tumbe el inbox; y que un filtro vacío no viaje como `?band=`, que no es «sin filtro» sino «los que tengan la banda vacía». Con `jsdom` y no un navegador real: lo que se prueba es la lógica del panel, no el motor de renderizado de Chrome. | ✅ Cerrada | 2026-08-09 |

| D73 | **El back-office pasa a Tailwind + shadcn/ui, y se sustituye el CSS a mano.** Lo que se gana no es estética: son componentes **accesibles de verdad** —foco visible, navegación por teclado, roles ARIA, cierre por Escape, anuncio a lectores de pantalla— que escribir a mano cuesta meses y sale mal; el diálogo de borrado y el desplegable de tono son el ejemplo. shadcn se copia al repositorio en vez de instalarse como dependencia, así que sigue sin haber una librería de diseño que imponga su calendario de versiones mayores: los componentes son nuestros y se editan. Dos ajustes sobre el tema por defecto: esquinas algo menos redondeadas (0.5 rem) y azul de marca en vez del gris neutro de serie, porque en una herramienta de trabajo la acción tiene que distinguirse de un borde a un metro. Hubo un tercero —base tipográfica de 13 px buscando densidad— y **se revirtió**: era ilegible, y sobre todo un `font-size` fijo en la raíz **pisa la preferencia del navegador**, así que a quien tiene el suyo en 20 px porque lo necesita se le devolvían 13 px igualmente. Sin esa declaración manda el navegador y la escala de shadcn recupera sus proporciones; si la densidad vuelve a hacer falta se ajusta en los componentes que la necesiten, no encogiendo el sistema entero. La temperatura de un lead (caliente/tibio/frío) son tokens del sistema, no clases sueltas: el mismo rojo significa lo mismo en la tabla, en el inbox y en el panel que venga. | ✅ Cerrada | 2026-08-09 |
| D74 | **Esta versión de shadcn va sobre Base UI, no sobre Radix.** No es un detalle de importación: cambia la API de composición —`render={<a/>}` en lugar de `asChild`— y el registro asume Next.js, así que el `Toaster` llegó dependiendo de `next-themes`, que en una aplicación de Vite no pinta nada. Se quitó y los colores del aviso salen de las variables CSS, que ya cambian solas con la clase `.dark`. Queda escrito porque es exactamente el tipo de suposición que hace perder una tarde a quien añada el siguiente componente. | ✅ Cerrada | 2026-08-09 |

| D75 | **El escalado avisa a una persona, y el aviso sale por evento, no dentro del turno.** Faltaba el consumidor: `HandoffCoordinator` publicaba `agent.handoff_requested` y no lo escuchaba nadie, así que el bot se callaba, el cliente leía «te paso con un asesor» y la conversación se quedaba muerta hasta que alguien abriera el inbox por casualidad — se perdía justo al cliente más interesado. Se resuelve con un puerto `Notifier` (asunto y cuerpo: lo único que comparten correo, Slack o SMS), `SmtpNotifier` con Mailpit en desarrollo y `LogNotifier` cuando no hay `SMTP_HOST`, porque la aplicación tiene que seguir arrancando sin configurar nada. Va por evento para sacar un SMTP que puede tardar cinco segundos del camino crítico del cliente. Y es el único handler del sistema que **relanza** al fallar: que no se anote un interés en el CRM se puede perder; que nadie atienda a quien pidió hablar con una persona, no — al lanzar, el outbox reintenta con backoff y acaba en `dead-letter` con su motivo. | ✅ Cerrada | 2026-08-09 |
| D76 | **La comprobación de que un escalado avisa es de FLUJO, no unitaria.** Un handler que nadie registra pasa todos sus tests unitarios: eso es exactamente lo que ocultó el fallo de D75 durante nueve fases. El test arranca donde arranca el problema —un cliente escribiendo «quiero hablar con un asesor»— y recorre orquestador, política de escalado, evento, bus y notificador. Para que sea posible, el harness registra las mismas suscripciones que el módulo al arrancar. La regla que queda: **cuando un fallo consista en que dos piezas no están conectadas, el test tiene que cruzar la conexión.** | ✅ Cerrada | 2026-08-09 |

| D77 | **Embeddings reales: un solo adaptador para OpenAI y para todo lo compatible.** Faltaba desde F8 —el `switch` lanzaba «llega en F8» y F8 trajo los modelos de chat pero no estos—, así que la base de conocimiento buscaba con los vectores del simulador. Mismo criterio que en el chat (D51): `baseUrl` e `id` por configuración en vez de una clase por proveedor. Tres cosas que el adaptador **no** hace, y las tres a propósito: no rellena ni recorta vectores cuando la dimensión no cuadra (falla con un mensaje que dice qué modelo elegir), no se fía del orden del array —ordena por el `index` de la respuesta, porque un lote desordenado emparejaría cada fragmento con el vector de otro sin dar error— y no cachea. La dimensión que no cuadra se devuelve como `InternalError` y no como `UpstreamError`: el proveedor no ha fallado, se le ha configurado mal, y reintentarlo no lo arregla. | ✅ Cerrada | 2026-08-09 |
| D78 | **Se pide `encoding_format: "float"` explícitamente.** Sin decir nada, el SDK de OpenAI solicita los vectores en base64 y los descodifica él. Contra OpenAI funciona; contra un servicio compatible que no implemente ese formato —Ollama, vLLM— la respuesta llegaría en floats y el SDK la leería como base64: vectores basura **sin ningún error**, un buscador que responde con total seguridad y se equivoca siempre. Se descubrió instrumentando la petición real en las pruebas, no leyendo documentación, y es la razón por la que el adaptador expone un `fetch` inyectable. | ✅ Cerrada | 2026-08-09 |
| D79 | **La suite de contrato distingue «comparte vocabulario» de «entiende sinónimos».** La segunda es opcional (`semantic: false`) y el simulador no la pasa: hace un *hashing trick* sobre la bolsa de palabras, así que «mascotas» y «perro» le quedan tan lejos como «mascotas» e «impuestos». Se declaró como excepción explícita en vez de rebajar la prueba para todos, porque esa propiedad —encontrar la respuesta cuando el cliente no usa las palabras del documento— es exactamente lo que se paga en un proveedor real y lo único que el carril vectorial aporta sobre el full-text que ya funciona sin él. | ✅ Cerrada | 2026-08-09 |
| D80 | **Al conectar un canal, comprobar las credenciales informa pero NO bloquea.** El caso de uso pregunta al canal si el token sirve —`ChatChannel.verifyCredentials?()`, opcional porque la consola no tiene nada que comprobar— y guarda igual aunque la respuesta sea que no, devolviendo `verified: false` y el motivo. La asimetría es el argumento: si la comprobación falla y aun así guardamos, la inmobiliaria ve un aviso claro y vuelve a pegar el token; si la comprobación falla **por nuestra parte** —Meta caída, un endpoint que cambió— y eso bloqueara el alta, nadie podría conectar WhatsApp y no habría forma de saltárselo desde el producto. Un sistema que se auto-bloquea por su propio diagnóstico es peor que uno que avisa. Se verifica *antes* de guardar (para que comprobar sirva de algo) pero se guarda *igual* (para que no sea un obstáculo). | ✅ Cerrada | 2026-08-09 |
| D81 | **La verificación LEE el número; no envía un mensaje de prueba.** Enviar gastaría una conversación facturable y le llegaría a una persona real. De la respuesta no se lee un solo campo, solo el código de estado: el contrato del que dependemos es «una lectura autenticada del nodo responde 2xx, y si no, Meta explica por qué en el sobre de error que ya modelamos en F6». Interpretar campos de un cuerpo que no hemos visto contra una cuenta real sería inventarse la API. **Pendiente de confirmar contra un número verificado** — ver Decisiones abiertas. | ⚠️ Sin verificar contra Meta | 2026-08-09 |
| D82 | **La ruta de alta es específica del canal; el caso de uso, genérico.** `POST /api/channels/whatsapp` en vez de un `POST /api/channels/accounts` con un `credentials` abierto: cada proveedor pide secretos distintos, y un endpoint genérico trasladaría al navegador la responsabilidad de saber qué claves espera cada adaptador. La traducción del contrato público a las claves del adaptador ocurre en el borde HTTP; `ConnectChannelAccountUseCase` recibe un diccionario y no sabe qué es WhatsApp. Conectar Telegram añadirá una ruta, no un caso de uso. | ✅ Cerrada | 2026-08-09 |
| D83 | **Los canales que el despliegue sabe operar viajan en el listado (`available`).** Sin esto, la pantalla no podría distinguir «esta inmobiliaria aún no ha conectado WhatsApp» de «esta instalación no tiene la app de Meta configurada» (D31), y pintaría un formulario que no puede funcionar. Deducirlo de si ya hay cuentas daría exactamente el caso contrario al que importa. | ✅ Cerrada | 2026-08-09 |
| D84 | **Panel y API en un solo proceso, un solo origen.** La API sirve el bundle del back-office y cualquier ruta que no sea suya devuelve el `index.html` para que React Router la resuelva. No es comodidad: la cookie de sesión es `SameSite=Lax`, que es lo que la protege de CSRF, y servir el panel desde otro dominio obligaría a `SameSite=None` —debilitando esa protección— o a montar un dominio padre compartido. Con un solo origen la cookie sigue igual, CORS deja de intervenir en producción y se despliega una cosa en vez de dos. La lista de prefijos de backend (`/api/`, `/webhooks/`, `/health`, `/metrics`) es lo que impide que un `GET /api/leadz` mal escrito devuelva el `index.html` con un 200 y el error real quede enterrado bajo un fallo de parseo. | ✅ Cerrada | 2026-08-09 |
| D85 | **El `index.html` se lee al arrancar y se sirve desde memoria.** Con `sendFile` las cabeceras de caché las pone el plugin estático a partir de su `maxAge`, y el `index.html` acababa marcado `immutable` por un año. Como lleva dentro las referencias a los bundles con hash, el siguiente despliegue dejaría navegadores pidiendo ficheros que ya no existen: una pantalla en blanco que «se arregla sola» al rato y que nadie sabe reproducir. Lo cazó el test antes que un usuario. De paso, si el fichero falta el proceso no arranca. | ✅ Cerrada | 2026-08-09 |
| D86 | **El artefacto de producción nunca se había ejecutado, y no funcionaba.** Dos fallos que el `build` no detectaba: `@agentinmobi/contracts` se publica como fuente TypeScript y quedaba *externo*, así que el bundle pedía a Node importar un `.ts` en tiempo de ejecución; y el cliente generado de Prisma —CommonJS, con `require` dinámico para su motor— se empaquetaba dentro de un bundle ESM y moría con «Dynamic require of "fs" is not supported». Ninguno aparecía en desarrollo porque ahí corre `tsx`. Se arregla empaquetando los paquetes del workspace (`noExternal`) y sacando Prisma del bundle mediante el especificador estándar `@prisma/client` en vez de una ruta relativa a `src/generated`. **El trabajo de CI que arranca la imagen existe por esto**: construir no demuestra nada, ejecutar sí. | ✅ Cerrada | 2026-08-09 |
| D87 | **Las migraciones son un paso de despliegue, no del arranque.** `dist/release.js` provisiona el rol sin `BYPASSRLS`, aplica `prisma migrate deploy` y recrea los índices que Prisma no sabe expresar —el HNSW de pgvector, que trata como deriva y tira en cada migración—. Migrar al arrancar haría que dos instancias levantándose a la vez compitieran por el mismo `ALTER TABLE`; el fallo saldría en uno de cada diez despliegues, la peor frecuencia posible para diagnosticar nada. Por eso el CLI de Prisma pasa a `dependencies`: la imagen tiene que poder migrarse a sí misma. | ✅ Cerrada | 2026-08-09 |
| D88 | **PDF y Word: el caso peligroso es el que NO falla.** Un reglamento escaneado es un PDF de imágenes sin una sola letra dentro. Se ingiere sin error, se indexa sin fragmentos, y el agente responde «no lo sé» durante meses sin que nadie relacione una cosa con la otra. Por eso un PDF sin texto se **rechaza** con un mensaje que nombra el problema («es un documento escaneado») y la salida (OCR o pegarlo a mano). No se hace OCR: adivinar mal el contenido de un reglamento es peor que no aceptarlo. El puerto `TextExtractor` pasa a recibir `Buffer` y a ser asíncrono — un binario no sobrevive a una cadena UTF-8, y leer un PDF no es instantáneo. | ✅ Cerrada | 2026-08-09 |
| D89 | **De Word se extrae Markdown, no texto plano.** En un `.docx` los títulos son estilos, y `convertToMarkdown` los convierte en `#`. Esos `#` son justo lo que la política de troceado usa para saber bajo qué epígrafe cae cada párrafo: la diferencia entre citar «sesenta días de preaviso» suelto y citarlo bajo «Terminación anticipada» (misma razón que D26 para conservar el Markdown). El conversor escapa la puntuación —«tres meses\.»— y eso se deshace: da igual mientras el texto solo se indexe, y deja de dar igual en cuanto el agente **cita ese fragmento a un cliente por WhatsApp**. `convertToMarkdown` existe pero no está en los tipos de `mammoth`; se declara la firma exacta que se usa, comprobada ejecutándola. | ✅ Cerrada | 2026-08-09 |
| D90 | **Lo que se guarda es el texto extraído, no el archivo original.** Reindexar —al cambiar de modelo de embeddings, D25— vuelve a leer ese artefacto, así que es rápido, determinista y no depende de que la librería de PDF siga comportándose igual. Obliga a que el reindexado extraiga con `EXTRACTED_MIME` y no con el tipo del documento: pasarle `application/pdf` mandaría texto plano al lector de PDF y dejaría el documento en fallo cada vez que alguien lo reindexara. El tipo original se conserva en la fila, para poder enseñarlo. | ✅ Cerrada | 2026-08-09 |
| D91 | **Los binarios viajan en base64 dentro del JSON, no en `multipart`.** Decisión de proporción: los documentos de una inmobiliaria son reglamentos, no vídeos, y el tope son unos megas. `multipart` ahorraría el 33 % de base64 a cambio de otro analizador de peticiones, otro camino de errores y otro sitio donde equivocarse con los límites. La decodificación ocurre en el borde HTTP —el caso de uso solo sabe de bytes— y **se comprueba re-codificando**, porque Node no falla ante un base64 inválido: descarta lo que no reconoce y devuelve lo que pueda, así que un archivo cortado a la mitad se indexaría a medias sin un solo error. El `bodyLimit` de Fastify sube a 12 MB para que un archivo del tamaño permitido no lo rechace el servidor antes de llegar a la validación que sabe explicar por qué. | ✅ Cerrada | 2026-08-09 |
| D92 | **En Fly.io la máquina no se apaga sola.** `auto_stop_machines = false` y `min_machines_running = 1`: dentro del proceso corren el relay del outbox y los recordatorios de visita, y una máquina dormida solo despierta con una petición HTTP — no porque toque mandar un recordatorio. Ahorrar unos céntimos a cambio de recordatorios que no salen y reintentos que no se ejecutan no es un ahorro. Las migraciones van en `release_command`, que Fly ejecuta en una máquina temporal antes de publicar la versión nueva: si fallan, el despliegue se aborta y sigue corriendo la anterior (D87). La sonda es `/health/live` y no `/health/ready`, porque decide si **reiniciar**: con la de readiness, un Postgres caído provocaría un bucle de reinicios. | ✅ Cerrada | 2026-08-09 |
| D93 | **En Fly, los archivos van a un volumen; el disco de la máquina es efímero.** Ahí vive el texto extraído de cada documento, que es lo que lee el reindexado: sin volumen, el primer despliegue posterior a subir un reglamento lo dejaría imposible de reindexar, y cualquiera subido y aún sin indexar se perdería. Las búsquedas seguirían funcionando —los fragmentos y sus vectores están en Postgres—, y eso es justo lo que haría el fallo difícil de ver. El volumen ata la aplicación a una máquina, y está bien: escalar exige antes resolver los trabajos duplicados y los límites de ritmo en memoria; cuando llegue, el sustituto es S3/R2 detrás de `FileStorage`. Un `entrypoint` da permiso sobre el punto de montaje y baja a `node` con `su-exec`: los volúmenes se montan después de construir la imagen y pertenecen a root, así que el `chown` del Dockerfile no los alcanza. | ✅ Cerrada | 2026-08-09 |
| D94 | **Invitación y restablecimiento son la MISMA operación.** Las dos comprueban un token de un solo uso y fijan una contraseña; lo único que cambia es de dónde venía el enlace, y eso solo afecta al texto. Un `RedeemUserTokenUseCase` y una `RedeemPage`, no dos de cada: duplicarlo daría dos implementaciones del mismo control de seguridad, y tarde o temprano una de las dos se quedaría sin un arreglo. Comparten también tabla, distinguidas por `purpose`. | ✅ Cerrada | 2026-08-09 |
| D95 | **Los tokens se guardan hasheados y el enlace nunca vuelve a salir.** Mismo trato que la cookie de sesión (D34): 256 bits de entropía, en base solo el hash, y la fila se conserva al consumirse para poder responder «este enlace ya se usó» en vez de «no existe». Emitir uno nuevo invalida los anteriores — un correo de recuperación de hace semanas no puede seguir abriendo la cuenta. Canjear **no** abre sesión: sería cómodo y significaría que quien tenga el enlace entra sin escribir jamás la contraseña. Y fijar contraseña cierra todas las sesiones de esa persona, que es lo único que da sentido a restablecerla. | ✅ Cerrada | 2026-08-09 |
| D96 | **«He olvidado mi contraseña» responde igual exista la cuenta o no.** Va contra el instinto de devolver un 404, y es deliberado: contestar distinto convertiría el formulario en un comprobador de qué correos trabajan en qué inmobiliaria, probando uno a uno. Va en la propia pantalla de acceso y no en otra, porque allí ya están escritos la inmobiliaria y el correo — que es exactamente lo que hace falta. | ✅ Cerrada | 2026-08-09 |
| D97 | **Nadie concede más de lo que tiene, y nadie se toca a sí mismo.** Un ADMIN no puede crear ni modificar a un ADMIN o a un OWNER: si pudiera, su rol sería en la práctica el de propietario y la distinción no significaría nada. Y nadie cambia su propio rol ni se desactiva, para que la última persona con acceso no pueda cerrarse la puerta y obligar a entrar por consola. La regla se comprueba en el caso de uso, no en la ruta: el navegador solo decide qué botones pintar. | ✅ Cerrada | 2026-08-09 |
| D98 | **Sin correo configurado, la invitación devuelve el enlace a quien invita.** El producto tiene que funcionar sin SMTP —como todo lo demás—, y callar dejaría una invitación que nadie puede aceptar. Enseñárselo a quien invita no concede nada nuevo: ya tenía potestad para dar ese acceso. Solo se devuelve cuando el envío **no** salió, y nunca en el camino de recuperación, donde quien lo pide es anónimo. | ✅ Cerrada | 2026-08-09 |
| D99 | **`ApplicationOptions.overrides`: una única costura de pruebas.** Permite sustituir una pieza de infraestructura —el notificador— dejando en pie todo lo demás. Sin ella, probar un flujo que manda un correo obligaría a levantar un servidor SMTP en cada test o a no probarlo, y lo segundo es lo que suele pasar. `main.ts` no la pasa nunca. | ✅ Cerrada | 2026-08-09 |
| D100 | **El alta de una inmobiliaria no recibe contraseñas: emite un enlace.** `dist/onboard.js` crea la inmobiliaria, su propietario y su canal de consola, y termina imprimiendo un enlace de invitación. Una contraseña pasada por línea de comandos queda en el historial del intérprete, en los registros del orquestador y en la pantalla de quien mire. Es repetible —si ya existe, reemite el enlace—, lo que lo convierte además en la vía de recuperación cuando alguien se queda fuera y no hay correo configurado. Va en el bundle porque `tsx` y el seed no existen en la imagen de producción: sin este comando no habría forma de entrar en lo recién desplegado. | ✅ Cerrada | 2026-08-09 |
| D101 | **El notificador de log tapa los tokens.** Cuando solo mandaba avisos de escalado, volcar el cuerpo entero era inofensivo; desde que manda invitaciones y restablecimientos, ese cuerpo lleva **una credencial que abre una cuenta**, y los logs se envían fuera, se guardan meses y los lee gente que no tendría por qué entrar en el panel de un cliente. El enlace completo sigue llegando a quien hizo la operación —la pantalla de equipo lo enseña, el comando de alta lo imprime—; el log no es el sitio. | ✅ Cerrada | 2026-08-09 |
...
5. **Confirmar la lectura de verificación contra Meta (D81).** La comprobación al conectar una línea está probada contra un servidor local que imita la Graph API: eso fija la petición que enviamos —método, ruta y `Authorization`— pero no demuestra que Meta responda como suponemos. Es la única suposición viva sobre una API externa en todo el sistema, y por diseño su fallo degrada a `verified: false` en vez de romper el alta. Se cierra con el primer número real.
6. **Desconectar o desactivar una línea.** Rotar el token ya funciona —volver a conectar el mismo número lo reemplaza—, pero no hay forma de dar de baja una línea desde el panel. La entidad ya tiene `disable()`/`enable()`; faltan el caso de uso y la ruta. No se hizo a la vez para no mezclar dos cambios, y porque desactivar la cuenta de consola rompería el simulador: hace falta decidir antes si el canal de consola es desactivable.
