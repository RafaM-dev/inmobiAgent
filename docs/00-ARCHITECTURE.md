...
| **F8 — Providers reales** | `OpenAIProvider`, `AnthropicProvider`, `OllamaProvider`. Contract tests compartidos. | `LLM_PROVIDER=openai` y nada más cambia. |
| **F9 — Producción** | Guardrails avanzados, rate limiting por tenant, control de coste, OpenTelemetry, backups, cifrado de credenciales, RLS, dashboards, evaluación automática de calidad. | SLOs definidos y medidos; runbook de incidentes. |
